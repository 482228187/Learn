...
创建指针 i指向  nums1 末位数字，j指向  nums2 末位数字。
i, j 数字相加，用进位就用  carry 来记录进位值，无则为0
若产生进位，则当前数字为 (i+j)%10 的值。
若遍历过程中， nums1 或 nums2 当前已无数字，则用 0补位来计算。
...
如何才能做到空间复杂度是 O(1) 呢？脑袋开始疯狂打转，但没有思路。没办法，退回原点。

心想，如果使用暴力破解法，该如何解决，很简单：每次从数组中取一个数，记为 cur，然后从剩下的数中查找，如果找不到，则 cur 即为要找的那个数。这

作者：Jerry银银
链接：https://leetcode.cn/problems/single-number/solutions/5118/xue-suan-fa-jie-guo-xiang-dui-yu-guo-cheng-bu-na-y/ 
...
MyQueue.prototype.pop = function () {
    // 当前出队栈中没有数据则将入队栈中的数据放到出队栈
    if (this.ou.length == 0) {
        while (this.in.length != 0) {
            this.ou.push(this.in.pop())
        }
    }
...
MyQueue.prototype.peek = function () {
 // 当前出队栈中没有数据则将入队栈中的数据放到出队栈
   if (this.ou.length == 0) {
    ...
    return this.ou[this.ou.length - 1]
};
...
MyQueue.prototype.empty = function () {
    return !(this.in.length || this.ou.length)
};
...
作者：御三五 🥇
链接：https://leetcode.cn/problems/min-stack/solutions/691969/fu-zhu-zhan-zui-xiao-zhan-by-demigodliu-wnpk/ 
...
MinStack.prototype.push = function(val) {
    ...
    if(len ===0 || this.minStack[len-1]>val){
...
从左到右枚举卖出价格 prices[i] ，那么要想获得最大利润，我们需要知道第 i天之前，股票价格的最小值是什么，也就是从 prices[0]到 prices[i−1]的最小值，把它作为买入价格，这可以用一个变量 minPrice维护。
...
每次都是向左数字会变小，向下数字会变大，有点和二分查找树相似。二分查找树的话，是向左数字变小，向右数字变大。
...
var searchMatrix = function(matrix, target) {
   // 从右上角开始查找，如果目标值大于它就向下查找，row++，否则，向左边查找，col--
...
作者：笨猪爆破组
链接：https://leetcode.cn/problems/reverse-linked-list-ii/solutions/663796/shou-hua-tu-jie-92fan-zhuan-lian-biao-ii-r9sf/ 
